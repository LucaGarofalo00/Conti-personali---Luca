    ...
    <div className="mb-6 bg-amber-50 border border-amber-300 rounded-xl p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <p className="font-semibold text-amber-800 mb-1">Database non aggiornato</p>
          <p className="text-sm text-amber-700 mb-2">
            Mancano queste colonne: <span className="font-mono text-xs">{missingColumns.join(', ')}</span>.
            Esegui il file <span className="font-mono">supabase-migration.sql</span> in Supabase SQL Editor.
          </p>
          <details className="text-xs text-amber-700">
            <summary className="cursor-pointer font-medium">Mostra SQL da eseguire</summary>
...
alter table variable_expenses add column if not exists needs_confirmation boolean not null default false;`}</pre>
          </details>
        </div>
