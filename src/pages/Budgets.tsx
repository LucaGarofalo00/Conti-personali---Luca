    ...
    <div>
      ...
      <div className="mb-8">
        ...
        {budgets.length === 0 ? (
          ...
          <div className="space-y-3">
            {budgets.map(b => (
              <div key={b.id} className={`bg-white rounded-xl border border-slate-200 p-4 flex items-center justify-between ${!b.is_active ? 'opacity-50' : ''}`}>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">{cur(Number(b.amount) * 4.33)}/mese stimato</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-lg font-semibold text-amber-600">{cur(Number(b.amount))}<span className="text-xs font-normal text-slate-400">/sett</span></span>
                  <button onClick={() => { setEditingBudget(b); setBudgetForm({ name: b.name, amount: Number(b.amount), fund_id: b.fund_id || '' }); setShowBudgetModal(true) }} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400"><Pencil className="w-4 h-4" /></button>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => { setEditingVar(null); setVarForm({ name: '', estimated_amount: 0, frequency: 'weekly', fund_id: '', category: 'trasporti' }); setShowVarModal(true) }} className="flex items-center gap-2 px-3 py-1.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition text-sm font-medium">
            <Plus className="w-4 h-4" /> Aggiungi
          </button>
        </div>
              ...
              <div key={v.id} className={`bg-white rounded-xl border border-slate-200 p-4 flex items-center justify-between ${!v.is_active ? 'opacity-50' : ''}`}>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 capitalize">{v.category} · {v.frequency === 'weekly' ? `${cur(Number(v.estimated_amount) * 4.33)}/mese` : 'mensile'}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => { setEditingVar(v); setVarForm({ name: v.name, estimated_amount: Number(v.estimated_amount), frequency: v.frequency, fund_id: v.fund_id || '', category: v.category }); setShowVarModal(true) }} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400"><Pencil className="w-4 h-4" /></button>
